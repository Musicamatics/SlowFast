...
print("This will take a while - dataset has 1.28M training images and 50K validation images")
...
def save_split(split_name, split_data):
    ...
    for idx, sample in enumerate(tqdm(split_data, desc=f"Saving {split_name}")):
        ...
        # Create class directory (using same naming as ImageNet-100)
...
# Convert both splits
print("\n" + "="*60)
print("CONVERTING IMAGENET-1K DATASET")
print("="*60)
...
print(f"\nExpected:")
print(f"  Train:      ~1.28M images (~1,281 per class)")
print(f"  Validation: ~50K images (~50 per class)")
print(f"\nTotal size: ~140-160 GB (after conversion)")
print(f"\nFolder naming: class_000, class_001, ..., class_999")
print(f"Compatible with SlowFast (same as ImageNet-100)!")
print("="*60)
